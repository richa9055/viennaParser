...
# Logger
logging.basicConfig(filename="logfile.log", level=logging.INFO, format= '[%(asctime)s] {%(pathname)s:%(lineno)d} %(levelname)s - %(message)s', datefmt='%H:%M:%S')
...
for filename in os.listdir(htmlFolder):
    ...
    if 'TM' in filename and '.htm' in filename:
        ...
        for i in range(total_marks):
            ...
            try:    
                ...
                proprietor = soup.find('span', id='ContentPlaceHolder1_MGVSearchResult_LblVProprietorName_'+str(i)).text
                app_num = (soup.find('span', id='ContentPlaceHolder1_MGVSearchResult_lblapplicationnumber_'+str(i)).text).strip(' ')
                ...
                vienna_code = (soup.find('span', id='ContentPlaceHolder1_MGVSearchResult_LblViennaCode_'+str(i)).text).strip(' ')
            ...
            try:
                ...
                sql = "insert into mark_vienna_map(application_no, vienna_code, last_updated_date) values(%s, %s, %s)"
                val = (app_num, vienna_code, formatted_date)
                db_cursor.execute(sql, val)
                db_connection.commit()
                logging.info("Info Added to Table mark_vienna_map: "+app_num+','+vienna_code+','+formatted_date)
                count_total_marks+=1


                sql = "insert into mark_details(application_no, last_updated_date, wordmark, proprietor, class, status, error, image_url) values(%s , %s, %s, %s, %s, %s, %s, %s)"
                val = (app_num, formatted_date, wordmark, proprietor, class_num, status, error, img_url)
                db_cursor.execute(sql, val)
                db_connection.commit()
                logging.info("Info Added to Table mark_details: "+app_num+','+formatted_date+','+wordmark+','+proprietor+','+class_num+','+status+','+error+','+img_url)
        ...
        if count_total_marks == total_marks:
            ...
            print(count_total_marks, total_marks)
